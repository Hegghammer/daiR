...
#'
#' @param file path to a pdf or image file.
...
#' @param imageless logical; if \code{TRUE}, removes images from the response
#'   and raises the sync page limit from 15 to 30. Defaults to \code{FALSE}.
...
#' are sent to Document AI as raw \code{application/pdf} bytes. The Document AI
#' synchronous endpoint accepts up to 15 pages (30 when \code{imageless = TRUE}).
#' A warning is issued if a PDF exceeds the relevant limit. Extract the text
#' from the response object with \code{text_from_dai_response()}. Inspect the
#' entire response object with \code{httr::content()}.
#' @export
#'
#' @examples
#' \dontrun{
...
#' )
#'
#' response <- dai_sync("long_doc.pdf", imageless = TRUE)
#' }
dai_sync <- function(
  file,
  ...
  if (!(is.logical(imageless) && length(imageless) == 1 && !is.na(imageless))) {
    stop("Invalid imageless parameter. Must be TRUE or FALSE.")
  }

  if (extension == "pdf") {
    check_pdf_pages(file, imageless)
  }
  ...
  # encode
  if (extension == "pdf") {
    encoded_file <- base64enc::base64encode(file)
  } else {
    encoded_file <- img_to_binbase(file)
  }

  # create json request body
  mime <- if (extension == "pdf") "application/pdf" else "image/png"
...
#' processor from the release candidate channel.
#' @param skip_rev whether to skip human review; "true" or "false"
#' @param field_mask optional comma-separated string specifying which Document
#' output fields to include. For example, omit page images by listing the text
#' and page fields you need without including `pages.image`.
#' @param loc a two-letter region code; "eu" or "us"
...
#'
#' # Exclude embedded page images by selecting only the required fields:
#' dai_async(my_files,
#'   field_mask = paste(
#'     c(
#'       "text", "uri", "shardInfo", "pages.pageNumber",
#'       "pages.dimension", "pages.layout", "pages.detectedLanguages",
#'       "pages.blocks", "pages.paragraphs", "pages.lines", "pages.tokens"
#'     ),
#'     collapse = ","
#'   )
#' )
...
dai_async <- function(
  ...
  if (!is.null(field_mask) && !(
    is.character(field_mask) &&
      length(field_mask) == 1 &&
      !is.na(field_mask) &&
      nzchar(trimws(field_mask))
  )) {
    stop("Invalid field_mask parameter.")
  }
  ...
  # create json request body
  gcs_output_config <- list("gcsUri" = dest_folder_uri)

  if (!is.null(field_mask)) {
    gcs_output_config$"fieldMask" <- field_mask
  }

  req <- list(
    "inputDocuments" = list("gcsDocuments" = list("documents" = doc_list)),
    "documentOutputConfig" = list("gcsOutputConfig" = gcs_output_config),
    "skipHumanReview" = skip_rev
  )
...
#' Check PDF page count against Document AI sync limits
#' @noRd
check_pdf_pages <- function(file, imageless) {
  max_pages <- if (isTRUE(imageless)) 30 else 15
  n_pages <- pdftools::pdf_info(file)$pages
  if (n_pages > max_pages) {
    warning(glue::glue("PDF has {n_pages} pages; Document AI sync limit is {max_pages} pages (imageless = {tolower(imageless)}). Request will likely fail."))
  }
}
