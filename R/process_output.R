...
make_hocr <- function(type,
    ...
    if (type == "sync") { message("hocr from reponse objects is still under development") } 
        ...
        for (i in seq_along(page_coords)) {
            id <- paste0("page_", i)
            x1 <- page_coords[[i]][["x"]][[1]]
            if (is.na(x1)) x1 <- 0
            y1 <- page_coords[[i]][["y"]][[1]]
            if (is.na(y1)) y1 <- 0
            x2_page <- page_coords[[i]][["x"]][[3]]
            y2_page <- page_coords[[i]][["y"]][[3]]
            bbox <- paste0("bbox ", x1, " ", y1, " ", x2_page, " ", y2_page)
