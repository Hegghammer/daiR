...
Open the navigation menu on the left again. Scroll down, almost to the bottom, till you see "Document AI" (under the group heading "Artifial intelligence"). Click "Document AI".

Now click the blue button labelled "Create processor". On the next page, choose the "Document OCR" processor type. A pane should open on your right where you can choose a name for the processor. Call it what you like; the name is mainly for your own reference. Select the location from which you will be using it, then click create.
...
Open your `.Renviron` file by calling `usethis::edit_r_environ()`. Add `DAI_PROCESSOR_ID="<paste your processor id here"` on a separate line. Save `.Renviron` and restart RStudio.   

That's it. If these things are in place, you will be auto-authenticated and ready to go when you attach (load) the `daiR` package.^[After attaching, you can check that the token was obtained with the function `dai_has_token()` or by viewing the `.auth` object directly. If credentials are missing, it's likely because `daiR` could not find the `GCS_AUTH_FILE` variable in your `.Renviron` file. Return to the vignette on "Setting up Google Storage" and make sure steps 5 and 6 are covered. Alternatively, you can, if you prefer, obtain an access token by another method and feed it into the token parameter of the `dai_sync()` and `dai_async()` functions.]
...
In asynchronous (offline) processing, you don't send DAI the actual document, but rather its location on Google Storage so that DAI can process it "in its own time". While slower than synchronous OCR, it allows for batch processing. The `daiR` function `dai_async()` is vectorized, so you can effectively send thousands of documents off for processing with a single line of code. In the following, we will just use a single document, but the filename can be replaced with a vector of almost any length.   
...
A few words about this function. Its core parameter, `files` (in this case, `"CIA.pdf"`), tells DAI which file(s) to process. `files` can be either a single filepath or a vector or list of filepaths. You can process either .pdf, .gif, or .tiff files. Your `files` vector can contain a mixture of these three file formats.
