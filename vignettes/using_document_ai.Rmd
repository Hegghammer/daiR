...
In asynchronous (offline) processing, you don't send DAI the actual document, but rather its location on Google Storage so that DAI can process it "in its own time". While slower than synchronous OCR, it allows for batch processing. The `daiR` function `dai_async()` is vectorized so you can send multiple files with one line of code. DAI's (rate limit)[https://cloud.google.com/document-ai/quotas] is currently 50 files per request and maximum 5 batch requests in concurrent processing. But a multi-page pdf counts as one file, so a request can effectively contain hundreds or even thousands of pages (up to a maximum of 10 000 pages in concurrent processing). For this vignette, however, we'll just use a single document; the same as in the previous example.
