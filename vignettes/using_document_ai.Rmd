...
Open the navigation menu on the left again. Scroll down, almost to the bottom, till you see "Document AI" (under the group heading "Artifial intelligence"). Click on "Document AI".
...
Although `dai_async()` takes batches of files, it is constrained by Google's (rate limits)[https://cloud.google.com/document-ai/quotas]. For the moment, a `dai_async()` call can contain a maximum of 50 files, and you can not have more than 5 batch requests and 10 000 pages (a multi-page pdf counts as one file) undergoing processing at any one time.

Therefore, if you're looking to process a very large batch, you need to spread the `dai_async()` calls out over time. There are several ways of doing this, but the simplest is probably to make a function that sends files off individually with a small wait in between. Say we have a vector called `big_batch` containing thousands of filenames. First we would make a function like this:
...
This, of course, will hold up your console for a long time, so it may be worth doing in the background as an RStudio [job](https://blog.rstudio.com/2019/03/14/rstudio-1-2-jobs/). 
